...
## 0. 画像配置の厳格な存在意義（Why Image?）と装飾画像の禁止

エージェントは「AI画像生成オプションが選ばれたから」といって、全スライドに漫然と画像を配置してはならない。
ビジネスプレゼンテーションにおいて、**単なる雰囲気出しのイメージ画像（アイキャッチ、中身のない3Dキューブ、抽象的な光る球等）はノイズであり、スライドの品格と説得力を破壊する（Anti-Decorative Imagery Rule）**。

### ① 画像配置を正当化する「3大利用シーン」
画像は以下のいずれかに該当し、**「その画像がなければ視覚的証明が成り立たない」場合にのみ配置**すること：
1. **【UI・プロダクト・ハードウェアの具現化】**: 実際の管理画面、モックアップ、物理製品の構造提示
2. **【現場の実態・生々しいペイン】**: 現場の書類の山、業務混乱、レガシーシステムの物理的ボトルネックの証明
3. **【構造的対比（AS-IS vs TO-BE）】**: 左の混乱した現場写真 vs 右の整然とした自動化システムの視覚対照

### ② スライド種別ごとの画像許容マトリクス
| スライドパターン | 画像配置の可否 | 理由と推奨表現 |
| :--- | :---: | :--- |
| **【表紙】** | △（限定的） | デジタル庁のような「CSSグラデーション＋公式ベクターロゴ」が最も品格が高い。 |
| **【エグゼクティブサマリ】** | **✕（完全禁止）** | **画像配置は厳禁。横スプリット4段（左表題・右巨大結論）の純粋なテキスト＆数値で勝負する聖域。** |
| **【目次（アジェンダ）】** | **✕（完全禁止）** | CSSカードグリッド＆章トラッカーで構成。画像はノイズ。 |
| **【課題・打ち手型】** | ◯（推奨: 1枚） | 現場ペイン（左）または新システムの具体概念（右）のいずれか1枠に絞って配置。 |
| **【トレードオフ比較表】** | **✕（完全禁止）** | ハーベイボール（●◕◐◔○）とハイライト枠で構成。画像を入れると表が壊れる。 |
| **【ステップ・時系列フロー】** | **✕（完全禁止）** | 横並びステップカード＆★Gate関門で構成。画像は不要。 |
| **【全体像・階層マッピング】** | △（限定的） | 原則は文字が読めるCSS/SVGスタック図。画像にする場合は各層の解説が外側で完全可読であること。 |

### ③ コード最優先の原則（Code-over-Pixels）
論理構造、フロー、階層関係は、AI生成画像（ピクセル）ではなく、**Tailwind CSSグリッド、カード、境界線、インラインSVG（ベクター）で描くこと**。コードで描かれた図解こそが、最も鮮明で、文字が読め、編集可能で、意味を持つ。
...
<taste id="cinematic_photorealistic">
