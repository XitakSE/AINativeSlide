...
### 4. AI生成画像の要否 & 配置スロット選択（Anti-Decorative Guardrails）
- [A] **画像不要（CSS/SVG表現・最優先推奨）**: すべてTailwindカード・グリッド・インラインSVG図解で構成（情報密度と視認性が最も高い）
- [B] **特定スライドのみ限定配置（推奨: 1〜2枚厳選）**:
  - スタイル選択: [B-1] リアル実写 / [B-2] 漫画コミック / [B-3] 3Dアイソメトリック / [B-4] フラットベクター
  - ※エグゼクティブサマリ、アジェンダ、比較表、フロー図への画像配置は情報伝達を阻害するため自動除外（No-Image Zone）されます。
  - ※画像を採用する場合、枠比率と完全同期した専用プロンプトで個別生成（トリミングなし・Base64インライン埋め込み）
