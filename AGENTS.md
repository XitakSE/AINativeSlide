...
├── SKILL.md                        # Antigravity / Agent Skill definition file (Builder Engine)
...
├── themes/                         # Corporate design tokens
│   └── default_theme.json          # Standard IBM Carbon / corporate theme token
...
### Rule 6: Design Tokens & Parity with AINativeSlide-Template-Builder
- **Theme JSON Token Architecture**:
  - `AINativeSlide-Template-Builder` (Repo: `../AINativeSlide-Template-Builder/`, Skill: `.agents/skills/ainativeslide-template-builder/`) extracts corporate branding into structured design tokens (`themes/theme.json` or `themes/default_theme.json`).
  - Design tokens are deterministically injected into `assets/template_base.html` via `scripts/apply_theme.py` or `scripts/assemble_deck.py --theme <path_to_theme.json>`.
...
- **Structured Wireframe Catalog (Strict Language Separation)**: Prefer the 6 enterprise wireframe patterns documented in [`references/slide-patterns.md`](references/slide-patterns.md). Strictly separate presentation by user language: when interacting in Japanese, use **ONLY intuitive Japanese names** (`【課題・打ち手型】`, `【トレードオフ比較表】`, `【スコープ境界線】`, `【全体像・階層マッピング】`, `【ステップ・時系列フロー】`, `【落とし穴・NG/OK対比】`) without exposing developer IDs; when interacting in English, use **ONLY English pattern names** (`[Problem & Solution]`, `[Comparison Matrix]`, `[Scope & Boundary]`, `[Architecture Mapping]`, `[Sequential Workflow]`, `[Pitfalls & Best Practices]`).
