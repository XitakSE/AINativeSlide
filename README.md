...
2. [実装済み機能一覧](#実装済み機能一覧)
   ...
   - [4. LocalStorage 自動保存＆復元（未保存ロスト防止）](#4-localstorage-自動保存復元未保存ロスト防止)
   - [5. 全画面プレゼンテーションモード（スライドショー）](#5-全画面プレゼンテーションモードスライドショー)
   - [6. ブラウザ直接推敲 & インライン書式バー](#6-ブラウザ直接推敲--インライン書式バー)
   - [7. 全体フォントサイズ動的スケーリング](#7-全体フォントサイズ動的スケーリング)
   - [8. 企業公式デザインテンプレート（CI/VI統一・自社PPTX連携）](#8-企業公式デザインテンプレートcivi統一自社pptx連携)
...
- **厳格な検査項目**:
  ...
  - 必須UI ID（ヘッダー、モード切替タブ、プレゼンモーダル、印刷ゼロマージン）の完全性
...
### 4. LocalStorage 自動保存＆復元（未保存ロスト防止）
...
### 6. ブラウザ直接推敲 & インライン書式バー
- **直接編集**: スライド上のすべてのテキストをクリックしてそのままキーボードで編集可能。
- **選択時ミニ書式バー**: テキストを範囲選択すると、太字・フォントサイズ・蛍光ペンハイライト（黄/緑）・書式解除が直上に出現。

### 7. 全体フォントサイズ動的スケーリング
- **全体文字サイズ調整**:
  - ヘッダーの `[A-]` `100%` `[A+]` コントローラーで、全スライドのフォントサイズを 85%〜125% の範囲で滑らかに一括調整。発表会場の広さやプロジェクターの視認性に応じて即座に最適化できます。

### 8. 企業公式デザインテンプレート（CI/VI統一・自社PPTX連携）
