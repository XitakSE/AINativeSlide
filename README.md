...
### ✅ 実装完了マイルストーン
- [x] **完全分離型 発表者ツール（Speaker View）＆ 登壇タイマー**: `BroadcastChannel` による外部通信ゼロ・手元モニター専用ウィンドウ（経過/残り時間タイマー、カンペ拡大縮小、次スライド予告、忠実スライドレンダリング）。
- [x] **LocalStorage 自動保存＆復元・リセット**: テキスト編集、発表メモ、カラーパレット、文字サイズを自動保存し、リロード時の消失を防止。
- [x] **ヘッダーUIのセグメントタブ化 ＆ 位置ブレ・干渉の完全解消**: 「✏️ 編集」と「🎬 発表・出力」のタブをスライドタイトルのすぐ右隣に配置し、画面幅縮小時も右側機能ボタンとの被りを完全に防ぎつつ、タブ切替時の0pxブレを維持。
- [x] **動的スマートGrill規則**: 指示済みの前提条件（比率・枚数等）の重複質問を自動スキップ。
- [x] **インラインSVGビジネスチャート & 比較テーブル標準対応**: 棒・折れ線複合グラフおよび推奨列ハイライト付き比較表パターンの完備。
- [x] **全社CI・ブランディング体系化**: 企業ロゴ、機密区分バッジ、フッター定位置レイアウトの標準化。
- [x] **AI自律修正型 自動テストシステム（CLI自動検証スクリプト）**: `scripts/verify_slide.py` によるスライド枚数・連番・メタボックス・文字数/はみ出しヒューリスティック・印刷CSSの高速静的解析と、エラー検出時のAI自動自己修復ループ。人間がテスト結果を見ることなく、100%パスした完成品のみを納品。
- [x] **社内Skill機能への完全統合（手動ナレッジ登録不要）**: リポジトリ自身のリソース（`resources/` と `scripts/`）をAIが直接参照・実行する、完全手ぶらの社内運用体制の確立。
- [x] **ドキュメントの一元標準化**: 非標準な個別マニュアルファイルを排除し、`README.md` 1本であらゆる生成AI（ChatGPT, Claude, Gemini, 社内エージェント）の利用と設定を網羅。
